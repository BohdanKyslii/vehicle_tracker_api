...
├── IMPORT_1C_SPEC.md     # Специфікація формату вивантажень з 1С (Фаза 12, реалізовано)
...
└── warehouse/             # ⚠️ НЕ цей проєкт — окремий, непов'язаний Django-застосунок
                           ...
                           # Не в git (2026-09-06) — лишається лише локально, є в .gitignore.
```
...
| **Import з 1С** | Завантаження реєстру накладних (РУБІН/ЄСП/ОПТ) через `/import_file/` | ✅ реалізовано (Фаза 12) |
| **Admin API (`/panel`)** | Керування користувачами — ролі, підтвердження, Telegram-лінк | ✅ реалізовано |
