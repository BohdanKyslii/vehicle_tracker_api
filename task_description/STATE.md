...
- ~~Набрати руками код `Фази 12`~~ — код набрано користувачем, ця
  сесія (2026-09-06) звірила його з `Кроком 16.1-16.8` гайду й
  виправила 2 реальні баги, знайдені при рев'ю (комітнуто в окрему
  гілку `faza_16_import_file`, запушено в `origin`):
  - `apps/waybills/importers/base.py` — `ParsedRow` мав поле
    `store_name` продубльоване двічі замість другого поля
    `customer_name` (яке зникло) — обидва парсери й `importing.py`
    передають/читають `customer_name`, тому будь-який виклик
    `import_file` падав з `TypeError: unexpected keyword argument
    'customer_name'` ще до того, як дійти до запису в БД. Також
    прибрано зайвий невикористаний `import decimal`.
  - `apps/waybills/views.py` — `get_queryset()` в кінці повертав
    `super().get_queryset()` замість зібраного `qs` (копія
    елізії-плейсхолдера з гайду `# ... без змін ...` +
    `return super().get_queryset()`, яка в гайді позначала "код тут
    без змін", а не буквальний рядок для вставки) — усі фільтри
    (`customer_id`/`product_id`/`legal_entity`/`delivery_channel`/
    `date_from`/`date_to`) мовчки не застосовувались.
  - Дрібне: `xlrd==2.0.1` у `requirements.txt` переставлено в
    алфавітний порядок (був між `python-dotenv` і `sqlparse`, має
    бути між `tzdata` і `yarl`).
  `python manage.py check` чистий, `ParsedRow` вручну перевірено
  інстанціюванням з тими самими kwargs, що й реальні парсери — падав
  до фіксу, працює після. **Ще не перевірено**: реальний
  `POST /import_file/` на живих файлах (Крок 16.8 п.1-4) і
  фактичне застосування фільтрів через `GET` (Крок 16.8 п.5 —
  авторизація вже перевірялась раніше, сам факт фільтрації — ні).
